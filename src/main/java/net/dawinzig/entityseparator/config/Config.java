...
public class Config {
    ...
    public boolean saveRule(Path relPath, Rule rule) {
        try {
            NbtIo.writeCompressed(rule.asNbt(), rulesPath.resolve(relPath));
        } catch (IOException e) {
    ...
    private void loadRule(Path relPath, File file) {
        try {
            Config.RULES.put(relPath, new Rule(NbtIo.readCompressed(file.toPath(), NbtTagSizeTracker.ofUnlimitedBytes())));
